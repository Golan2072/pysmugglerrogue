...
def hex_contents(column, row, starmap, cursor_tuple):
    if column == cursor_tuple[0] and row == cursor_tuple[1]:
        contents = "\u001b[31m@\u001b[32m"
    elif starmap[column][row] != None:
        contents = "\u001b[33m*\u001b[32m"
    else:
        contents = " "
    return contents
...
class Main_screen (Game_screen):
    ...
    def render_screen(self):
        ...
        print(
            f"PySmuggler {self.game.version}\n\nBy Omer Golan-Joel\n\nMain Screen")
...
class Navigation_screen (Game_screen):
    def __init__(self, game):
        ...
        self.particular_menu_options = {
            "J": ("Jump", self.jump), "I": ("Information", self.information)}
    ...
    def render_starmap(self):
        ...
        for row in range(1, 11):
            star_string += f"/ {hex_contents(1, row, self.starmap, cursor)} \\{hex_number(2, row-1, self.starmap)}/ {hex_contents(3, row, self.starmap, cursor)} \\{hex_number(4, row-1, self.starmap)}/ {hex_contents(5, row, self.starmap, cursor)} \\{hex_number(6, row-1, self.starmap)}/ {hex_contents(7, row, self.starmap, cursor)} \\{hex_number(8, row-1, self.starmap)}/ \n"
            star_string += f"\\{hex_number(1, row, self.starmap)}/ {hex_contents(2, row, self.starmap, cursor)} \\{hex_number(3, row, self.starmap)}/ {hex_contents(4, row, self.starmap, cursor)} \\{hex_number(5, row, self.starmap)}/ {hex_contents(6, row, self.starmap, cursor)} \\{hex_number(7, row, self.starmap)}/ {hex_contents(8, row, self.starmap, cursor)} \\ \n"
    ...
    def jump(self):
        ...
        if origin_hex.column == destination_hex.column and origin_hex.row == destination_hex.row:
        ...
        elif self.starmap[destination_hex.column][destination_hex.row] == None:
            ...
            print(
                f"Your FTL Rating is {self.game.starship.ftl_rating}, which is insufficient for this jump.")
            ...
            self.game.character.location = (
                destination_hex.column, destination_hex.row)
            self.game.starship.current_fuel -= self.game.starship.fuel_per_parsec * \
                calculate_distance(origin_hex, destination_hex)
    ...
    def information(self):
        print(self.starmap[self.game.character.location[0]]
              [self.game.character.location[1]]["Name"])
        input("Press Enter to proceed")
        self.menu()
...
class Shipyard_screen (Game_screen):
    ...
    def render_screen(self):
        ...
        print(
            f"Fuel {self.game.starship.current_fuel} tons out of {self.game.starship.max_fuel} tons")
        print(
            f"Refuel cost {50*(self.game.starship.max_fuel - self.game.starship.current_fuel)} Terran Credits")

    def refuel(self):
        cost = 50*(self.game.starship.max_fuel -
                   self.game.starship.current_fuel)
